        ...
        function activate() {
            if($stateParams.transId){
                TransService.getTrans($stateParams.transId).then(function(data) {
                    $timeout(function () {
                        $scope.trans = data;
                        $scope.trans.created_time.date = (new Date(data.created_time.date)).toISOString().substring(0, 10);
                        $scope.trans.updated_time.date = (new Date(data.updated_time.date)).toISOString().substring(0, 10);
